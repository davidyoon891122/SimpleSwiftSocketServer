...
const croom = io.of('/c');
const userList = [
  { "id": "1", "name": "ns", "isConneted": false },
  { "id": "2", "name": "jm", "isConneted": false },
  { "id": "3", "name": "jh", "isConneted": false },
  { "id": "4", "name": "hj", "isConneted": false }
]
const chatRoom = [
...
router.get('/', function (req, res) {
  room.emit('test', { 'Hello': 'World', 'test': 'good' });
...
router.get('/getChatList/:name', (req, res) => {
  ...
  const list = []
  chatRoom.forEach(element => { if (element.name == name) { list.push(element) } }); res.send(list)
})
